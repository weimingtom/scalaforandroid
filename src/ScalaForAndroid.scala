...
class SetupScalaTask extends Task {
    ...
    private def addProperties {
        getProject getProperty "target" match {
            case "android-3" => addAndroid15properties
            case "android-7" => addAndroid21properties
            case _ => throw new BuildException("Unsupported android build target. Only 1.5 and 2.1 are supported.")
        }
    }
